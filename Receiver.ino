...
RF24 radio(9, 10); const byte address[5] = {0x12, 0x23, 0x45, 0x67, 0x89};
Adafruit_PCD8544 display = Adafruit_PCD8544(7, 6, 5, 4, 3); float list[3]; int b_int = 1; 
const unsigned char MarilynMonroe0 [] PROGMEM = {
  ...
  0xff, 0xff, 0xf0}; const unsigned char MarilynMonroe [] PROGMEM = {
  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xf0, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
  0x30, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
  0x00, 0x10, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
  0x00, 0x00, 0x10, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
  ...
  0x10, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
  0x00, 0x10, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
  0xff, 0xff, 0xf0}; void setup() { Serial.begin(9600); radio.begin(); radio.openReadingPipe(0, address); radio.setChannel(0x76); 
  radio.setPALevel(RF24_PA_MAX); radio.setDataRate(RF24_250KBPS); radio.startListening(); radio.enableDynamicPayloads(); 
  /*If there is data loss during transmission, try removing this code.*/ radio.powerUp(); display.begin(); display.setContrast(57);
  display.clearDisplay(); } void loop() { if (radio.available()) { if(b_int == 1) { int b_time = millis(); do { display.display();
  display.clearDisplay(); display.drawBitmap(0, 0,  MarilynMonroe, 84, 48, BLACK); /*display.setTextSize(1); 
  display.setTextColor(BLACK); display.setCursor(0,0); display.println("Alexander Kibarov"); display.setTextColor(BLACK); 
  display.setCursor(0,8); display.println("Welcome To The Back");*/} while(millis()-b_time < 5000); b_time = millis();
  do { display.display(); display.clearDisplay(); display.drawBitmap(0, 0,  MarilynMonroe0, 84, 48, BLACK);
  /*display.setTextSize(1); display.setTextColor(BLACK); display.setCursor(0,0); display.println("Alexander Kibarov");
  display.setTextColor(BLACK); display.setCursor(0,8); display.println("Welcome To The Back");*/
